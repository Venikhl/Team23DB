...
spark-submit --master yarn preprocess_data.py \
    --output_path "$OUTPUT_PATH"

# Step 3: Train Model 1
echo "Training Model 1..."
spark-submit --master yarn train_model1.py \
...
spark-submit --master yarn train_model2.py \
    --train_path "$TRAIN_PATH" \
    --test_path "$TEST_PATH" \
...
pylint *.py > ../output/pylint_report.txt || true
